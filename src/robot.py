...
class Robot:
    ...
    def __rotateLeft(self):
        ...
        self.__r_motor.runNegative(-0.3)
    ...
    def __rotateRight(self):
        ...
        while ((self.__l_motor.position() - prev_lEnc > -20) and (self.__r_motor.position() - prev_rEnc < 20)):
            if (self.__l_motor.position() - prev_lEnc <= -20):
                self.__l_motor.brake()
            if (self.__r_motor.position() - prev_rEnc >= 20):
                self.__r_motor.brake()
